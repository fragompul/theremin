...
export class Theremin {
  ...
  #animate() {
    ...
    const c1x = canvas.width / 4;
    const c1y = canvas.height / 4;
    const c2x = 3*canvas.width / 4;
    const c2y = canvas.height / 4;
    const c3x = canvas.width / 4;
    const c3y = 3*canvas.height / 4;
    const c4x = 3*canvas.width / 4;
    const c4y = 3*canvas.height / 4;
    ...
    if (centro != null){
      ...
      } else if(Math.abs(Math.sqrt((centro.x-c2x)**2+(centro.y-(c2y-20))**2)-60) < margenDistancia){
        this.puntosDentro2.push({ ...centro,tiempoCreacion: tiempoActual});
      } else if((centro.x-c3x) + (centro.y-c3y) < margenDistancia && Math.abs(centro.x-c3x) < 50){
        this.puntosDentro3.push({ ...centro,tiempoCreacion: tiempoActual});
      } else if((centro.x-c4x) - (centro.y-c4y) < margenDistancia && Math.abs(centro.x-c4x) < 50){
    ...
    if (this.puntosDentro1.length >= ratioPuntosDentro*contadorPuntosActivo && check){
      this.exito1 = true;
    ...
    } else if (this.puntosDentro2.length >= ratioPuntosDentro*contadorPuntosActivo && check){
      this.exito2 = true;
      document.body.classList.remove("grey-bg");
      document.body.classList.add("green-bg");
      // Crear el mensaje de "Congratulations" solo si no existe ya
    ...
    } else if (this.puntosDentro3.length >= ratioPuntosDentro*contadorPuntosActivo && check){
      this.exito3 = true;
      document.body.classList.remove("grey-bg");
      document.body.classList.add("green-bg");
      // Crear el mensaje de "Congratulations" solo si no existe ya
    ...
    } else if (this.puntosDentro4.length >= ratioPuntosDentro*contadorPuntosActivo && check){
      this.exito4 = true;
      document.body.classList.remove("grey-bg");
      document.body.classList.add("green-bg");
      // Crear el mensaje de "Congratulations" solo si no existe ya
    ...
    ctx.moveTo(c1x-70, c1y);
    ctx.lineTo(c1x+70, c1y);
    ctx.stroke();
    ...
    ctx.arc(c2x, c2y-20, 60, 0, Math.PI); // 50 is the radius of the circle
    ctx.stroke();
    ...
    ctx.moveTo(c3x-50, c3y+50);
    ctx.lineTo(c3x+50, c3y-50);
    ctx.stroke();
    ...
    ctx.moveTo(c4x-50, c4y-50);
    ctx.lineTo(c4x+50, c4y+50);
    ctx.stroke();
